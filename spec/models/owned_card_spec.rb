...
describe OwnedCard do
  ...
  describe ".add_from_mtg_db_card" do
    let!(:mtg_db_card) { FactoryGirl.create(:mtg_db_card) }

    it "returns an OwnedCard" do
      oc = OwnedCard.add_from_mtg_db_card(mtg_db_card, user)
    ...
    it "increments the number if the card is in the user's collection" do
      OwnedCard.add_from_mtg_db_card(mtg_db_card, user)
      oc = OwnedCard.add_from_mtg_db_card(mtg_db_card, user)
      
      expect(oc.number).to eq(2)
    end
  end
end
